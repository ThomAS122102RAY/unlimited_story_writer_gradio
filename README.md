...
* **☁️ 線上模型支援 (Online Models)**：**[新功能]** 支援直接連接 **xAI (Grok)**、**OpenAI (GPT-4o)**、**DeepSeek** 與 **OpenRouter**。
    * 即使沒有高階顯卡，也能透過 API 享受頂級模型的寫作能力。
    * 支援 **Grok-2/3**、**Claude 3.5 Sonnet** 等最新模型。
* **🛠️ 自動參數優化**：针对 **Grok Reasoning** 或 **OpenAI o1** 等推理模型，自動調整 API 參數避免報錯。
* **🧠 核心記憶 (Memory)**：新增不隨對話長度消失的記憶區，保持長線劇情不崩壞。
* **🖋️ 文風模仿 (Style DNA)**：**[強化]** 上傳你的歷史作品，AI 會自動分析你的筆觸細節，甚至可以建立專屬的「特化分身模型」。
* **🌍 語言切換**：支援繁中、簡中、英文、日文等多國語言輸出。
* **👁️ 感官權重**：針對視覺、聽覺、觸覺等五種感官進行百分比調整。

---

## ☁️ 如何連接線上模型 (Grok, OpenAI, DeepSeek)

如果你不想使用本地顯卡運算，或是想嘗試更聰明的線上模型，請依照以下步驟：

1. **切換服務商**：在介面左側「⚙️ 核心設定」中，找到 **「快速切換提供商」** 下拉選單。
2. **選擇平台**：目前支援 xAI (Grok), OpenAI, DeepSeek, OpenRouter 等。
3. **輸入金鑰 (API Key)**：
   * **Grok**: 前往 [xAI Console](https://console.x.ai/) 申請。
   * **OpenAI**: 前往 [OpenAI Platform](https://platform.openai.com/api-keys) 申請。
   * **DeepSeek**: 前往 [DeepSeek Platform](https://platform.deepseek.com/) 申請。
4. **測試連線**：填入 Key 後，點擊「📶 測試連線」，出現 ✅ 表示成功。
...
* **Q: 點擊生成後沒反應？**
  * A: 請檢查黑底視窗是否有報錯，並確認 Ollama 羊駝圖示是否在執行。若是使用線上模型，請檢查網路連線與 API Key 餘額。
* **Q: 換了本地模型後變得很慢？**
  * A: 如果模型太大（如 27b 模型）超出顯卡的記憶體 (VRAM)，電腦會改用 CPU 運算，速度會大幅下降。建議更換成較小的模型（如 9b 系列）。
* **Q: Grok 或 o1 模型報錯 400？**
  * A: 請確保更新到最新版本 (v3.1)，我們已修復了推理模型不支援某些參數的問題。
